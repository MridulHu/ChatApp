...
export default function Chat() {
  ...
  const onSignOut = () => {
    signOut(auth).then(() => navigation.replace("Login")).catch(error => console.log('Error logging out: ', error));
  };
  ...
  const onSend = useCallback((messages = []) => {
    ...
    const userWithDetails = {
      _id: auth.currentUser.uid,  // FIXED: Use UID instead of email
  ...
  return (
    <GiftedChat
      ...
      user={{
        _id: auth.currentUser.uid,  // FIXED: Match Firestore messages' user._id
        name: userData?.name || "Anonymous",
        avatar: userData?.avatar || `https://i.pravatar.cc/300?u=${auth.currentUser.uid}`
      }}
    />
